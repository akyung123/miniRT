...
/*
표면점 P 하나의 최종 색 = ambient + diffuse   (그늘이면 ambient만)
...
t_color	lighting(t_scene *scene, t_hit *rec)
{
	...
	// 7단계: P, n, 색을 따로 받지 않고 t_hit 하나로 받는다
	// 구든 평면이든 원기둥이든 이 함수는 똑같이 동작한다
	...
	// 광원을 등진 면(k = 0)은 어차피 diffuse가 0이니 그림자 광선을 쏠 필요도 없다
	// 광원을 향한 면만 '사이에 뭐가 있나' 확인하고, 막혔으면 ambient만 남긴다
	// ambient까지 0으로 만들면 그림자가 새까매진다
