...
shadow_test와 하는 일은 같고, 도형이 배열이 아니라 연결 리스트라는 것만 다르다
7단계에서 hit_scene이 생기면 이 반복문이 그걸로 바뀌고, 평면/원기둥도 그림자를 만든다
*/
int	in_shadow(t_scene *scene, t_vec3 p)
{
	...
	while (obj)
	{
		if (obj->type == OBJ_SPHERE
			&& hit_sphere(r, obj->data.sphere.center,
				obj->data.sphere.diameter / 2.0, T_MIN, dist) > 0.0)
			return (1);
		obj = obj->next;
	}
	return (0);
