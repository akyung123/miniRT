...
void	init_minirt(t_mlx *mlx)
{
	...
	// 메모리 안에 새로운 윈도우 생성
	mlx_new_window(mlx->mlx_ptr, WIDTH, HEIGHT, "miniRT");
	// 메모리 안에 새로운 이미지 생성
	mlx_new_image(mlx->img_ptr, WIDTH, HEIGHT);
	mlx->addr = mlx_get_data_addr(mlx->img_ptr, &mlx->bpp, &mlx->size_line, &mlx->endian);
}
