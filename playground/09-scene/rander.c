...
/*
픽셀 하나의 색 (9단계: 팀 규약 시그니처 그대로)

  t_color render_pixel(t_scene *scene, int x, int y);
  - width/height 는 인자로 안 받는다 → WIN_WIDTH / WIN_HEIGHT
  - 0.0 ~ 1.0 정규화 색을 돌려준다. 정수 변환은 출력 쪽 몫

1) 카메라 광선 — 이제 scene->camera (파서가 준 위치/방향/fov) 기준
2) hit_scene: 모든 도형 중 가장 가까운 교차 → rec (t, 점, 법선, 색)
3) 아무것도 안 맞으면 하늘
4) SHOW_RAY_DIR / SHOW_NORMAL 이면 방향·법선을 색으로 (-1~1 → 0~1)
5) 아니면 조명(ambient + diffuse + 그림자) 후 0~1로 자르기

7~8단계와 달라진 건 1) 한 줄뿐이다
도형/조명은 원래부터 scene 에서 읽고 있었으니, demo_scene 대신 parse_scene 이 채워주면 끝
*/
t_color	render_pixel(t_scene *scene, int x, int y)
{
	t_ray	r;
