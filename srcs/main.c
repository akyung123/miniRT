#include <stdlib.h>
#include "mlx.h"
#include "miniRT.h"
#include "output.h"
#include "move.h"

static int	has_rt_extension(const char *path)
{
	int	len;
	int	base;

	len = 0;
	while (path[len])
		len++;
	base = len;
	while (base > 0 && path[base - 1] != '/')
		base--;
	if (len - base <= 3)
		return (0);
	return (path[len - 3] == '.' && path[len - 2] == 'r'
		&& path[len - 1] == 't');
}

static int	check_args(int argc, char **argv)
{
	if (argc != 2)
	{
		ft_error("usage: ./miniRT <scene>.rt");
		return (0);
	}
	if (!has_rt_extension(argv[1]))
	{
		ft_error("scene file must have a .rt extension");
		return (0);
	}
	return (1);
}

int	main(int argc, char **argv)
{
	t_minirt	minirt;

	if (!check_args(argc, argv))
		return (EXIT_FAILURE);
	if (!parse_scene(argv[1], &minirt.scene))
		return (EXIT_FAILURE);
	if (!init_mlx(&minirt))
	{
		free_scene(&minirt.scene);
		return (EXIT_FAILURE);
	}
	render_all(&minirt);
	move_init(&minirt.scene.camera);
	mlx_hook(minirt.mlx.win_ptr, DESTROY_NOTIFY, DESTROY_MASK, close_hook,
		&minirt);
	mlx_hook(minirt.mlx.win_ptr, 2, 1L << 0, move_key_press, &minirt);
	mlx_key_hook(minirt.mlx.win_ptr, key_hook, &minirt);
	mlx_expose_hook(minirt.mlx.win_ptr, expose_hook, &minirt);
	mlx_loop_hook(minirt.mlx.mlx_ptr, move_loop_hook, &minirt);
	mlx_loop(minirt.mlx.mlx_ptr);
	return (EXIT_SUCCESS);
}
